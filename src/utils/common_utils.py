...
def batch_open(refs, mode='r'):
    ...
    if not isinstance(refs, (list, tuple)):
        refs = [refs]
    for f in refs:
        handlers.append(codecs.open(f, mode, 'utf-8'))
...
class VocabCopy(object):
    ...
    def __init__(self, vocab):
        ...
        assert isinstance(vocab, Vocab), 'vocab should be a instance of Vocab'

        # Word to word index and word frequence.
        max_n_words = vocab.max_n_words

        self._token2id_feq = {}

        for ww, vv in vocab._token2id_feq.items():
            if vv[0] < max_n_words:
                self._token2id_feq[ww] = vv

        self._id2token = dict([(ii[0], ww) for ww, ii in self._token2id_feq.items()])

        self.max_words_ori = max_n_words
    ...
    def update_vocab(self, word):
        self._token2id_feq[word] = (self._max_n_words, 0)
        self._id2token[self._max_n_words] = word
        self._max_n_words += 1

    def reset(self):
        # self._token2id_feq = {}
        token2id_feq = self._token2id_feq.copy()
        for ww, vv in token2id_feq.items():
            if vv[0] >= self.max_words_ori:
                self._token2id_feq.pop(ww)

        self._max_n_words = len(self._token2id_feq)
        self._id2token = dict([(ii[0], ww) for ww, ii in self._token2id_feq.items()])
        token2id_feq = {}
