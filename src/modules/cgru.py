...
class CGRUCell(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = BahdanauAttention(query_size=hidden_size, key_size=self.context_size, cover_size=cover_size)
    ...
    def forward(self,
        ...
        attn_values, attn_weights, coverage = self.attn(query=hidden1, memory=context,
                                                        cache=cache, mask=context_mask, coverage=coverage)
        hidden2 = self.gru2(attn_values, hidden1)

        return (hidden2, attn_values), hidden2, coverage, attn_weights
